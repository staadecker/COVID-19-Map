...
async function getGSReference(bucket, file) {
    try {
    ...
    return gsReference.child(file).getDownloadURL();
}

const bucketRequest = function(gsReference) {
...
async function obtainAndDisplayMaps() {
    var form_reference = await getGSReference(config['bucket'], "form_data.json");
    var form = bucketRequest(form_reference);
    var confirmed_reference = await getGSReference(config['bucket'], "confirmed_data.json");
    var confirmed = bucketRequest(confirmed_reference);
